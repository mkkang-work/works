    ...
    html2canvas(document.querySelector(".resultField_captureArea"), {
      ...
      $.ajax({ 
        ...
        dataType: 'json' 
          ...
          $('.resultImg').append('<a href="'+resultImg+'" target="_blank">[画像ページ(別窓)]</a>');
...
function showResult() {
  ...
  for (i=0; i<namMember.length; i++) {
    ...
    if(i % resulttablenm == 0){
      str += "<table>";
      str += "<tr><td style=\"color:#ffffff; background-color:#000; width:45px;\">順位<\/td><td style=\"color:#ffffff; background-color:#000;\">名前<\/td><\/tr>";
      var ip = i;
    }
